...
class Matrix {
    ...
    static createLookAt(position, target, up){
        let zAxis = this.vector();
        zAxis.sub(position, target);
        zAxis.normalize();

        let xAxis = this.vector();
        xAxis.cross(up, zAxis);
        xAxis.normalize();

        let yAxis = this.vector();
        yAxis.cross(zAxis, xAxis);

        let result = new Matrix(4, 4);

        result.setValue(0,0,xAxis.x);
        result.setValue(1,0,yAxis.x);
        result.setValue(2,0,zAxis.x);
        result.setValue(3,0,0);

        result.setValue(0,1,xAxis.y);
        result.setValue(1,1,yAxis.y);
        result.setValue(2,1,zAxis.y);
        result.setValue(3,1,0);

        result.setValue(0,2,xAxis.z);
        result.setValue(1,2,yAxis.z);
        result.setValue(2,2,zAxis.z);
        result.setValue(3,2,0);

        result.setValue(0,3,-xAxis.dot(position));
        result.setValue(1,3,-yAxis.dot(position));
        result.setValue(2,3,-zAxis.dot(position));
